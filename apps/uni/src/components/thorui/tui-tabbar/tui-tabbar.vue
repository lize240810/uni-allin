			...
			<view class="tui-tabbar-item" :class="{ 'tui-item-hump': item.hump }"
				...
				<view class="tui-icon-box" :class="{ 'tui-tabbar-hump': item.hump }">
					<image :src="current == index ? item.selectedIconPath : item.iconPath"
						:class="[item.hump ? '' : 'tui-tabbar-icon']" :style="{width: item.PathSize, height: item.PathSize}" v-if="!item.name"></image>
...
<script>
	...
	export default {
		...
		computed:{
			getActiveColor(){
				return this.selectedColor || (uni && uni.$tui && uni.$tui.color.primary) || '#273347';
			},
			getBadgeBgColor(){
